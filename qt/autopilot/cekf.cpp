...
void cEKF::estimateStates(tStateData &state, tRawSensorData &sensor)
{
    if( !mInitialized ) {
        init( state );
        mInitialized = true;
...
void cEKF::dirtyConvert(tStateData &state, tRawSensorData &sensor)
{
    state.gyro_x = sensor.gyro_x;
    state.gyro_y = sensor.gyro_y;
    state.gyro_z = sensor.gyro_z;

    state.accel_x = sensor.accel_x;
    state.accel_y = sensor.accel_y;
    state.accel_z = sensor.accel_z;

    state.mag_x = sensor.mag_x;
    state.mag_y = sensor.mag_y;
    state.mag_z = sensor.mag_z;
}

void cEKF::convertRawSensorData(tStateData &state, tRawSensorData &sensor)
{
    return;
    ...
    float temp = sensor.temperature*0.00357143 + 70.00;
    ...
    // Rate gyros
    svec.data[0][0] = (float)((float)sensor.gyro_x - (float)state.beta_p - (state.beta_p0 + state.beta_p1*temp + state.beta_p2*temp2 + state.beta_p3*temp3) );
    svec.data[1][0] = (float)((float)sensor.gyro_y - (float)state.beta_q - (state.beta_q0 + state.beta_q1*temp + state.beta_q2*temp2 + state.beta_q3*temp3) );
    svec.data[2][0] = (float)((float)sensor.gyro_z - (float)state.beta_r - (state.beta_r0 + state.beta_r1*temp + state.beta_r2*temp2 + state.beta_r3*temp3) );

    // Multiply gyro measurements by alignment matrix (fixes cross-axis alignment)
    //mat_mult( &state->gyro_cal, &svec, &svec );
    ...
    svec.data[0][0] = ((float)(sensor.accel_x - state.beta_acc_x));
    svec.data[1][0] = ((float)(sensor.accel_y - state.beta_acc_y));
    svec.data[2][0] = ((float)(sensor.accel_z - state.beta_acc_z));

    //mat_mult( &state->accel_cal, &svec, &svec );
    ...
    svec.data[0][0] = ((float)(sensor.mag_x - state.beta_mag_x));
    svec.data[1][0] = ((float)(sensor.mag_y - state.beta_mag_y));
    svec.data[2][0] = ((float)(sensor.mag_z - state.beta_mag_z));

    //mat_mult( &state->mag_cal, &svec, &svec );
...
void cEKF::predict(tStateData &gState)
{
    ...
    */
    ...
    // Project the error covariane ahead
